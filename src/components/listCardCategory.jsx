...
import t_shirt from "../assests/SvgImage/solid tshirt.svg"
import book from "../assests/SvgImage/solid book.svg"
...
const ListCardCategory = ({category}) => {
            ...
            <div className={`bg-white rounded p-[20px] ${shadows.shadowHeader} mt-12`}>
                <div className='flex justify-between items-center py-4 border-b border-[#E6ECF8]'>
                    <div className='flex gap-2'>
                        {category === 'Digital' ? <>
                            <img src={laptop} alt=""/>
                            <h3 className='text-[#141C2D] font-bold'>محصولات دیجیتال</h3>
                        </> : ''}
                        {category === 'Stationery' ? <>
                            <img src={book} alt=""/>
                            <h3 className='text-[#141C2D] font-bold'>لوازم تحریر</h3>
                        </> : ''}
                        {category === 'Clothes' ? <>
                            <img src={t_shirt} alt=""/>
                            <h3 className='text-[#141C2D] font-bold'>لباس </h3>
                        </> : ''}

                    </div>
                    <div className='flex gap-2'>
                        <Link to={`/${category}`} className='text-[#9DB5E4]'>دیدن محصولات بیشتر</Link>
                        <img src={leftArrow} className='mt-1' alt='leftArrow'/>
                    </div>
                </div>
                <div className='grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-[20px]'>

                    {productsData.filter((item) => item.category === category).map(data => (
                        <>
